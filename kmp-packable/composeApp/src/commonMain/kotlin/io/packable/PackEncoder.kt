...
class PackEncoder {
    companion object {
        ...
        fun <T> encodeList(value: List<T>, adapter: TypeAdapter<T>): ByteArray {
    ...
    private val buffer = EncodeBuffer(ByteArray(4096))
    ...
    fun toBytes(): ByteArray {
        checkBufferState()
        val bytes: ByteArray = buffer.hb.copyOf(buffer.position)
        recycle()
        return bytes
    }
    ...
    fun <T> putObject(index: Int, value: T?, packer: TypeAdapter<T>): PackEncoder {
        if (value == null) {
            return this
        }
        buffer.checkCapacity(6)
     ...
     */
    private fun <T> wrapObject(value: T?, packer: TypeAdapter<T>) {
        buffer.checkCapacity(2)
        if (value == null) {
            ...
            buffer.position += 2
            val pPack: Int = buffer.position
            packer.encode(this, value)
            val len: Int = buffer.position - pPack
            if (len <= 0x7fff) {
                buffer.writeShort(pLen, len.toShort())
            } else {
                buffer.checkCapacity(2)
                buffer.hb.copyInto(buffer.hb, pPack + 2, pPack, pPack + len)
                buffer.position += 2
